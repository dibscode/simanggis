...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-lg mx-auto bg-white p-6 rounded shadow">
        <h2 class="text-xl font-bold mb-6">Edit Sekolah</h2>
        @if ($errors->any())
        ...
        <form action="{{ route('schools.update', $school->school_id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="school_name" id="school_name" value="{{ old('school_name', $school->school_name) }}" class="w-full border px-3 py-2 rounded" required>
            </div>
            <div class="mb-4">
                <label for="address" class="block font-semibold mb-1">Alamat</label>
                <textarea name="address" id="address" rows="3" class="w-full border px-3 py-2 rounded" required>{{ old('address', $school->address) }}</textarea>
            </div>
            <div class="mb-4">
                <label for="contact_number" class="block font-semibold mb-1">No. Kontak</label>
                <input type="text" name="contact_number" id="contact_number" value="{{ old('contact_number', $school->contact_number) }}" class="w-full border px-3 py-2 rounded" required>
            </div>
